...
public interface EditableMapper extends EditableResolver, ObjectRelationalFactory {
	...
	default <K, V, S> List<Pair<K, V>> parseOptions(Class<? extends S> entityClass, Collection<? extends S> entitys,
		...
		if (StringUtils.isEmpty(keyName)) {
			// 主键
			keyField = fields.streamAll().filter((e) -> isPrimaryKey(entityClass, e.getGetter())).findFirst()
					.orElse(null);
		} else {
		...
		Field valueField = fields.streamAll().filter((e) -> isDisplay(entityClass, e.getGetter())).findFirst()
				.orElse(null);
